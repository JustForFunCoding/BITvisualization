...
Interface of 1D RU RQ BIT:
    - constructor (BitRuRq)
        ...
        - returns new instance of 1D RU RQ BIT, which is able to solve
            range update and range query operations, whose range
            is inside interval [1,size]
        - for example: bit = BitRuRq(11)
    - range update (update)
        - takes three integers: left, right, val
        - returns nothing, but 1D RU RQ BIT, on which is operation
            called is updated appropriately
