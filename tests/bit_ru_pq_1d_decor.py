...
class BitRuPq:
    """
    Class representing Binary indexed tree for set of operations
    ...
    def __init__(self, size: int = 0, do_animate=True, canvas="1", name=""):
        ...
        if self.animate and size > 0:
            ...
            self.draw.push_print("{}: {}".format(tree_name, self.tree[1:]))
    ...
    def updatep(self, idx: int, val: int, text_from_updater='',
        ...
        Args:
            ...
            spaces_from_updater:    helper indentation
                                        for nicer print in console
        ...
        if type(idx) is not int or type(val) is not int:
            ...
            while idx <= self.size:
                if self.animate:
                    ...
                    self.draw.push(self, TreeType.UpdateTree,
                                   BitRuPq.draw_update_tree,
                                   text_from_updatep, idx)
                self.tree[idx] += val
                idxold = idx
                idx = idx + lsb(idx)
                if self.animate:
                    self.draw.push(self, TreeType.UpdateTree,
                                   BitRuPq.draw_update_tree, text_from_updatep,
                                   idxold, idxold)
        elif self.animate:
    ...
    def updater(self, left: int, right: int, val: int, text_from_update='',
        ...
        Args:
            ...
            right:              index representing right boundary
                                    of the interval to be updated
            val:                value which is added to every element
        ...
        if self.animate:
            self.draw.push_print(f'{spaces_from_udpate}{tree_name}.'
    ...
    def queryp(self, idx: int, text_from_query='', result_name='result',
               tree_name='bit') -> int:
        """
        ...
        if self.animate:
            self.draw.push_print(f'{spaces_from_query}{tree_name}.'
                                 f'{queryp_info} starting')
            self.draw.push_print(f'{spaces}{result_name} = 0')
            self.draw.push(self, TreeType.QueryTree, BitRuPq.draw_query_tree,
                           f'{queryp_whole_info} starting, {result_name} := 0')
        ...
        if self.animate:
            self.draw.push_print(f'{spaces}{tree_name}.{queryp_info} = '
                                 f'{result_name} = {cumul_freq}')
            self.draw.push_print(f'{spaces_from_query}{tree_name}.'
                                 f'{queryp_info} finished')
            self.draw.push(self, TreeType.QueryTree, BitRuPq.draw_query_tree,
                           f'{queryp_whole_info} finished, {result_name} = '
                           f'{cumul_freq}', True, cumul_freq)
        return cumul_freq
    ...
    def print_tree(self) -> None:
        ...
        Returns:        nothing is returned, but the tree name with its content
    ...
    def get_single_freq(self, idx: int) -> int:
        """
        Operation of finding the value of single frequency
        ...
        res = self.queryp(idx)
        if self.animate:
            self.draw.push_print('{}get_single_freq({})={}'
                                 .format(self.name, idx, res))
            self.draw.push(self, TreeType.QueryTree,
                           BitRuPq.draw_query_tree, res)
        return res
    ...
    def get_single_freqs(self) -> list:
        ...
        if self.animate:
            self.draw.push_print("{}Item frequencies: {}"
                                 .format(self.name, item_freqs))
        return item_freqs
    ...
    def draw_query_tree(self, method_info='', last_draw=False,
                        cumul_freq=0, idx=None, highidx=None) -> None:
        if not self.animate:
            return
        ...
        self.animate = True
        # math.cos(math.pi / 4) * change
        change = 0.7071067811865476 * self.circle_diameter
        # clear the canvas
        self.draw.cls()
        self.draw.text_left(self.startx, self.starty, 'C:')
        self.draw.text_left(self.startx,
                            self.starty + 3 * self.circle_diameter, 'P:')
        self.draw.text_left(self.startx, max_y_1, 'V:')
        self.draw.text_left(self.startx, max_y_1 + 3 * self.circle_diameter,
                            self.name + "RU PQ tree")
        ...
        for i in range(self.size + 1):
            ...
            if i == 0:
                self.draw.text_in_circle_h(act_x, act_y, self.circle_diameter,
                                           cumul_freq, last_draw)
            else:
                self.draw.text_in_circle_h(act_x, act_y, self.circle_diameter,
                                           self.tree[i], idx is not None and
                                           i == idx)
            self.draw.text_in_circle_h(act_x, max_y_1, self.circle_diameter,
                                       freqs[i], idx is not None and i == idx)
            self.draw.text_center(act_x, max_y, i)
            if i > 0:
                parent_idx = i - lsb(i)
                parent_x, parent_y = \
                    self.get_circle_center_pos(parent_idx, TreeType.QueryTree)
                if highidx is not None and parent_idx == highidx and i == idx:
                    self.draw.vector_hi(parent_x + change, parent_y + change,
                                        act_x - change, act_y - change)
                else:
                    self.draw.vector(parent_x + change, parent_y + change,
                                     act_x - change, act_y - change)

    def draw_update_tree(self, method_info='',
                         highlight=0, highidx=None) -> None:
        if not self.animate:
            return
        ...
        self.draw.text_left(self.startx,
                            self.starty + 3 * self.circle_diameter, 'P:')
        self.draw.text_left(self.startx,
                            max_y + 3 * self.circle_diameter, 'V:')
        self.draw.text_left(self.startx, max_y_1 + 3 * self.circle_diameter,
                            self.name + "RU PQ tree")
        ...
        for i in range(1, self.size + 2):
            ...
            if i < end_index:
                ...
                self.draw.text_in_circle(act_x, act_y, self.circle_diameter,
                                         self.tree[i], width, color)
                self.draw.text_in_circle(act_x, max_y_1, self.circle_diameter,
                                         freqs[i], width, color)
                self.draw.text_center(act_x, max_y, i)
                # This prevents us from bad parent indices.
                # For instance, if we would have tree of size 11, then element
                # at index 8 would have parent at index 16, but it's
                # not possible because of the tree size.
                # That's why we have to pick minimum.
                parent_idx = min(i + lsb(i), end_index)
                parent_x, parent_y = \
                    self.get_circle_center_pos(parent_idx, TreeType.UpdateTree)
                if highidx is not None and i == highidx:
    ...
    def get_circle_center_pos(self, idx: int,
                              tree_type: TreeType) -> Tuple[float, float]:
        return self.startx + (idx + 1 if tree_type is TreeType.QueryTree
                              else idx) * self.circle_diameter * 3, \
               self.starty + self.get_vertex_height(idx, tree_type) * \
               self.circle_diameter * 3 + 40  # some space so 3x
