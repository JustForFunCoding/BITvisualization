...
class BitRuRq2d(BitRuPq2d):
    ...
    Attributes:
        ...
        size:       size of the trees, which is NOT changed
    ...
    def update(self, x1: int, y1: int, x2: int, y2: int, c: int) -> None:
        ...
        Args:
            ...
            c:      value which is added to every element inside
        ...
        if self.animate:
            ...
            self.draw.push(self, TreeType.UpdateTree, Bit2d.draw_update_tree,
                           None, None, c, f'{update_info} starting')

        if type(x1) is not int or type(x2) is not int or \
                type(y1) is not int or type(y2) is not int or \
                type(c) is not int:
            if self.animate:
                self.bitxy.draw.push_print(f'{spaces}invalid format')
                self.bitxy.draw.push(self.bitxy, TreeType.UpdateTree,
                                     Bit2d.draw_update_tree, None, None, c,
                                     f'{update_info} got invalid format')
        elif 1 <= x1 <= x2 <= self.size and 1 <= y1 <= y2 <= self.size:
            spaces = 4 * ' '
            self.bitxy.updater(x1, y1, x2, y2, c,
                               text_from_update, 'bitxy', spaces)
            self.bitx.updater(x1, y1, x2, y2, -c * (y1 - 1),
                              text_from_update, 'bitx', spaces)
            self.bitx.updater(x1, y2 + 1, x2, self.size, c * (y2 - y1 + 1),
                              text_from_update, 'bitx', spaces)
            self.bity.updater(x1, y1, x2, y2, -c * (x1 - 1),
                              text_from_update, 'bity', spaces)
            ...
            self.bitxy.draw.push_print(f'{spaces}invalid interval, '
                                       f'updating nothing')
            self.bitxy.draw.push(self.bitxy, TreeType.UpdateTree,
                                 Bit2d.draw_update_tree, None, None, c,
        ...
        if self.animate:
            ...
            self.draw.push(self, TreeType.UpdateTree, Bit2d.draw_update_tree,
                           None, None, c, f'{update_info} finished')
    ...
    def query(self, x: int, y: int) -> int:
        ...
        if type(x) is not int or type(y) is not int:
            if self.animate:
                ...
                self.bitxy.draw.push(self.bitxy, TreeType.QueryTree,
                                     Bit2d.draw_query_tree, None, None, 0,
                                     f'{query_info} got invalid format')
                self.bitxy.draw.push_print(f'{spaces_from_query}'
                                           f'{query_info} = 0')
            ...
            a = self.bitxy.queryp(x, y, text_from_query, 'a',
                                  'bitxy', spaces_from_query)
            b = self.bitx.queryp(x, y, text_from_query, 'b',
                                 'bitx', spaces_from_query)
            c = self.bity.queryp(x, y, text_from_query, 'c',
                                 'bity', spaces_from_query)
            d = self.biti.queryp(x, y, text_from_query, 'd',
                                 'biti', spaces_from_query)
            res = a * x * y + b * x + c * y + d
        elif self.animate:
            self.bitxy.draw.push_print(f'{spaces_from_query}out of range, '
                                       f'querying nothing')
        ...
        if self.animate:
            self.draw.push_print(f'    {query_info} = a*{x}*{y} + b*{x} + '
                                 f'c*{y} + d = {a}*{x}*{y} + {b}*{x} + '
                                 f'{c}*{y} + {d} = {res}')
            ...
            if d >= 0:
                ...
                self.draw.push(self, TreeType.QueryTree, Bit2d.draw_query_tree,
                               None, None, 0, f'{query_info} finished, result '
                                              f'= ({a}*{x}*{y}) + ({b}*{x}) + '
                                              f'({c}*{y}) - {-d} = {res}')
        return res
