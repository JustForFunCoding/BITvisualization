...
class TreeType(Enum):
    """
        There are 2 types of trees how can BIT be displayed:
            either as Query tree or as Update tree.
    """
    NoTree = -1
    QueryTree = 0
    UpdateTree = 1
...
if '__BRYTHON__' in globals():
    ...
    class CanvasDraw(object):
        ...
        def text_center(self, draw_x, draw_y, text_to_draw):
            self.ctx.fillText(str(text_to_draw), draw_x -
                              (self.ctx.measureText(text_to_draw).width / 2),
                              draw_y)
        ...
        def text_in_circle(self, draw_x, draw_y, circle_diameter, text_to_draw,
            ...
            ctx.fillText(str(text_to_draw), draw_x - measures.width / 2,
                         draw_y + (measures.actualBoundingBoxAscent +
                                   measures.actualBoundingBoxDescent) / 2)

        def text_in_circle_h(self, draw_x, draw_y, circle_diameter,
                             text_to_draw, highlight=False):
            self.text_in_circle(draw_x, draw_y, circle_diameter, text_to_draw,
                                draw_settings['highlight_line_width']
                                if highlight
                                else draw_settings['default_line_width'],
                                draw_settings['highlight_color']
                                if highlight
                                else draw_settings['default_color'])

        """ draw a rectangle centered at x, y """
        def text_in_rect_center(self, draw_x, draw_y, box_size, text_to_draw):
            self.ctx.beginPath()
            self.ctx.rect(draw_x - box_size/2, draw_y - box_size / 2,
                          box_size, box_size)
        ...
        """ draw a rectangle with upper left corner at
            x, y with optional highlight """
        def cell_2d_hi(self, x1, y1, box_size, text_to_draw, highlight=False):
            self.ctx.beginPath()
            self.ctx.rect(x1, y1, box_size, box_size)
            self.ctx.strokeStyle = draw_settings['highlight_color'] \
                if highlight else draw_settings['default_color']
            self.ctx.lineWidth = draw_settings['highlight_line_width'] \
                if highlight else draw_settings['default_line_width']
            self.ctx.stroke()
            measures = self.ctx.measureText(text_to_draw)
            self.ctx.fillText(str(text_to_draw),
                              x1 + (box_size - measures.width) / 2,
                              y1 + box_size -
        ...
        """ draw a vector from x1, y1 to x2,y2
            with default color and default width """
        def vector(self, x1, y1, x2, y2):
            self.vector_cw(x1, y1, x2, y2, draw_settings['default_color'],
                           draw_settings['default_line_width'])

        """ draw a vector from x1, y1 to x2,y2
            with default color and default width """
        def vector_hi(self, x1, y1, x2, y2):
            self.vector_cw(x1, y1, x2, y2, draw_settings['highlight_color'],
                           draw_settings['highlight_line_width'])

        """ draw vector animation starting at x1, y1 to x2, y2
            with 2 colors and percentage of highlight"""
        def vector_p(self, x1, y1, x2, y2, color1, color2, percent):
            dx = (x2-x1)*(percent/100)
            dy = (y2-y1)*(percent/100)
            if dx != 0 or dy != 0:
                self.vector_cw(x1, y1, x1+dx, y1+dy, color2,
                               draw_settings['highlight_line_width'])
            if x1 + dx != x2 or y1 + dy != y2:
                self.vector_cw(x1+dx, y1+dy, x2, y2, color1,
                               draw_settings['default_line_width'])
    ...
    class AnimatedLine(AnimatedObj):
        ...
        def animate(self, time):
            ...
            self.draw_support.vector_p(self.x1, self.y1, self.x2, self.y2,
                                       self.color1, self.color2, self.progress)
    ...
    class State1d(object):
        def __init__(self, type: TreeType = None):
            ...
            self.vectors.append(AnimatedLine(self.draw_support, 10, 10,
                                             100, 100))
            self.last_time = None
