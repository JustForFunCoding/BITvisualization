...
class BitPuRq(object):
    """
    Class for animated Binary Indexed Tree for set of operations
    ...
    def __init__(self, size: int = 0, do_animate=True, canvas="1", name="bit"):
        ...
        Why not only size? Because we have to index from 1, and we want
        to eliminate +- 1 bugs, so we let the array for tree to be [1...size],
        instead of possible [0...size-1].
        ...
        if self.animate and size > 0:
            self.draw.push_print("{}: {}".format(self.name, self.tree[1:]))
    ...
    def updatep(self, idx: int, val: int) -> None:
        ...
        if self.animate:
            ...
            self.draw.push(self, TreeType.UpdateTree, BitPuRq.draw_update_tree,
                           idx, idx, f'{updatep_info} starting')
        ...
        if type(idx) is not int or type(val) is not int:
            if self.animate:
                ...
                self.draw.push(self, TreeType.UpdateTree,
                               BitPuRq.draw_update_tree, idx, idx,
            ...
            while idx <= self.size:
                ...
                if self.animate:
                    self.draw.push(self, TreeType.UpdateTree,
                                   BitPuRq.draw_update_tree, idxold, idxold,
                                   updatep_info)
        elif self.animate:
            self.draw.push_print(f'{spaces}out of range, updating nothing')
            self.draw.push(self, TreeType.UpdateTree, BitPuRq.draw_update_tree,
                           idx, idx, f'{updatep_info} out of range')

        if self.animate:
            self.draw.push_print(f'{tree_name}.{updatep_info} finished')
            self.draw.push_print(f'{tree_name} after {tree_name}.'
                                 f'{updatep_info}: {self.tree[1:]}')
            self.draw.push(self, TreeType.UpdateTree, BitPuRq.draw_update_tree,
                           idx, idx, f'{updatep_info} finished')
    ...
    def queryr(self, idx: int) -> int:
        ...
        if self.animate:
            ...
            self.draw.push(self, TreeType.QueryTree, BitPuRq.draw_query_tree,
                           f'{queryr_info} starting, result := 0', 0)

        if type(idx) is not int:
            if self.animate:
                self.draw.push_print(f'{spaces}invalid format')
                self.draw.push(self, TreeType.QueryTree,
                               BitPuRq.draw_query_tree,
                               f'{queryr_info} got invalid format')
        elif 1 <= idx <= self.size:
            while idx > 0:
                if self.animate:
                    self.draw.push_print(f'{spaces}result += {tree_name}'
                                         f'[{idx}] (={self.tree[idx]})')
                    if self.tree[idx] >= 0:
                        self.draw.push(self, TreeType.QueryTree,
                                       BitPuRq.draw_query_tree,
                                       f'result := result + {self.tree[idx]} '
                                       f'in {queryr_info}', False,
                                       cumul_freq, idx)
                    else:
                        self.draw.push(self, TreeType.QueryTree,
                                       BitPuRq.draw_query_tree,
                ...
                if self.animate:
                    if self.tree[idxold] >= 0:
                        self.draw.push(self, TreeType.QueryTree,
                                       BitPuRq.draw_query_tree,
                                       f'result := result + '
                                       f'{self.tree[idxold]} in {queryr_info}',
            ...
            self.draw.push(self, TreeType.QueryTree, BitPuRq.draw_query_tree,
                           f'{queryr_info} out of range')

        if self.animate:
            self.draw.push_print('{}{}.queryr({}) = result = {}'
                                 .format(spaces, tree_name, idx0, cumul_freq))
            self.draw.push_print(f'{tree_name}.{queryr_info} finished')
            self.draw.push(self, TreeType.QueryTree, BitPuRq.draw_query_tree,
                           f'{queryr_info} finished, result = {cumul_freq}',
                           True, cumul_freq)
        return cumul_freq
    ...
    def get_single_freq(self, idx: int) -> int:
        ...
        if self.animate:
            self.draw.push(self, TreeType.QueryTree,
                           BitPuRq.draw_query_tree, res, idx)
        while predecessor != parent:
            res -= self.tree[predecessor]
            if self.animate:
                self.draw.push(self, TreeType.QueryTree,
                               BitPuRq.draw_query_tree, res, predecessor)
    ...
    def draw_query_tree(self, queryr_info='', last_draw=False,
    ...
    def draw_update_tree(self, highlight=0,
                         highidx=None, updatep_info='') -> None:
        if not self.animate:
            return
        ...
        for i in range(1, self.size + 2):
            ...
            if i < end_index:
                ...
                # For instance, if we would have tree of size 11,
                # then element at index 8 would have parent at index 16,
                # but it's not possible because of the tree size.
    ...
    def get_circle_center_pos(self, idx: int, tree_type: TreeType) \
            -> Tuple[float, float]:
        return self.startx + (
            idx + 1 if tree_type is TreeType.QueryTree else idx) * \
               self.circle_diameter * 3, \
               self.starty + self.get_vertex_height(idx, tree_type) * \
               self.circle_diameter * 3  # some space so 3x
